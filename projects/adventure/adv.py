...
# need to track : last room visited, current room, direction I can go, rooms ive been to, rooms ive checked fully. 
...
def findValidMoves(room, last, visited, checked):
    validMoves = {}

    for dir in room.getExits():
        player.travel(dir)
        if (player.currentRoom is not last) and (player.currentRoom not in visited) and (player.currentRoom not in checked):
            validMoves[dir] = player.currentRoom
        player.travel(reverseDir(dir))
    return validMoves

def getDirToValidMove(validMoves, stack):

    for (dir, room) in validMoves.items():
        if room == stack.peek():
            return dir

def getDirToRoom(start, dest):

    for dir in start.getExits():
        player.travel(dir)
...
def explore():
    ...
    s.enqueue(player.currentRoom)
    last = player.currentRoom

    while s.size() > 0:
        checking = s.peek()
        checked.add(checking)
        validMoves = findValidMoves(checking, last, visited, checked)
        
        if len(validMoves) > 0:
            nextRoomIndex = list(validMoves.keys())[0]
            nextRoom = validMoves[nextRoomIndex]

            s.enqueue(nextRoom)

            nextDir = getDirToRoom(player.currentRoom, s.peek())

            path.append(nextDir)

            last = player.currentRoom

            player.travel(nextDir)
            ...
            last = deStack
            reverseDir = getDirToRoom(deStack, s.peek())

            path.append(reverseDir)
            visited.add(player.currentRoom)
            player.travel(reverseDir)


# FILL THIS IN
traversalPath = explore()
